...
\usepackage{hyperref,amsthm,amsmath,amsfonts,enumerate}
...
\newcommand{\bNtrois}{\bZ_{\ge 3}}



\newcommand{\seg}[2]{\left[ #1, #2 \right]}
\newcommand{\sego}[2]{\left[ #1, #2 \right[}
\newcommand{\segro}[2]{\left] #1, #2 \right]}
...
\begin{theorem}[Hull and Dobell \cite{HullDobell62,Knuth97}] \label{thm:Hull-Dobbel}
  Let $a$, $c \in \bZ$, let $m \in \bNast$, and let $T\colon \bN \to \bZ$ be such that
  $m$ divides $T(n + 1) - a T(n) - c$ for every $n \in \bN$.
  % \begin{equation} \label{eq:T-aT+c}
  % T(n + 1) = a T(n) + c  
  % \end{equation}
  % for every $n \in \bN$.
  Assume that $\rho'(m)$ divides $a - 1$ and that $c$ and $m$ are coprime.
  For every $i$, $j \in \bN$,
  $m$ divides $T(i) - T(j)$ if, and only if, $m$ divides $i - j$.
\end{theorem}
\begin{theorem}[Hull and Dobell \cite{HullDobell62,Knuth97}] \label{thm:Hull-Dobbel}
  Let $a$, $c \in \bZ$, let $m \in \bNast$, and let $T\colon \bN \to \bZ$ be such that 
  $T(n + 1)$ is congruent to $a T(n) + c$ modulo $m$ for every $n \in \bN$.
  % \begin{equation} \label{eq:T-aT+c}
  % T(n + 1) = a T(n) + c  
  % \end{equation}
  % for every $n \in \bN$.
  Assume that $\rho'(m)$ divides $a - 1$ and that $c$ and $m$ are coprime.
  For every $i$, $j \in \bN$,
  $T(i)$ and $T(j)$ are congruent modulo $m$ if, and only if, $i$ and $j$ are congruent modulo~$m$.
\end{theorem}

\begin{theorem}
  Let $a \in \bZ$ and let $m \in \bNdeux$.
  There exists $t \in \{ 0, 1 \}$ such for every $n \in \bNast$, $a^n$ is not congruent to $t$ modulo~$m$.
\end{theorem}

\begin{proof}
  Assume that for each $t \in \bZ$, there exists $n \in \bNast$ such that $a^n$ is congruent to $t$ modulo~$m$.
  In particular,
  there exists $r \in \bNast$ such that $a^r$ is congruent to $0$ modulo $m$ and
  there exists $s \in \bNast$ such that $a^s$ is congruent to $1$ modulo~$m$.
  It follows from the former congruence that there exists a prime factor $p$ of $m$ such that $p$ divides $a$
...
\begin{theorem}
  Let $a$, $c$, $m$, and $T$ be as in Theorem~\ref{thm:Hull-Dobbel}.
  Assume
  that $m$ is not equal to $2$ and
  that for each $t \in \bZ$, there exists $n \in \bN$ such that $T(n)$ is congruent to $t$ modulo~$m$.
  Then, $\rho'(m)$ divides $a - 1$ and $c$ and $m$ are coprime.
\end{theorem}

\begin{proof}
  Since $m \ne 2$, $c + 1$ and $c - 1$ are not congruent modulo $m$,
  and thus there exists $\varepsilon \in \{ - 1, + 1 \}$ such that $T(0)$ is not congruent to $c + \varepsilon$ modulo~$m$.
  Let $k \in \bN$ be such that $T(k)$ is congruent to $c + \varepsilon$ modulo~$m$.
  Our choice of $\varepsilon$ precludes $k = 0$,
  and thus $a T(k - 1)$ is congruent to $\varepsilon$ modulo~$m$.
  Hence, $a$ and $m$ are coprime.
  In particular, if $m$ is even then $a - 1$ is even.
  Let $U \colon \bN \to \bZ$ be defined by:
  $$
  U(n) = (a - 1) T(n) + b = T(n + 1) - T(n) 
  $$
  for every $n \in \bN$.
  $$
  (a - 1) T(n) + b = T(n + 1) - T(n) = a^n ( T(1) - T(0)) = a^n \left( (a - 1) T(0) + b \right) 
  $$

  $$
  \frac{a^n U(0)  - b}{a - 1} = T(n)
  $$
...
\begin{exercise}
  ...
  \item Prove that $S(2 n, a) = (a + 1) S(n, a^2)$ (warning: $a = - 1$ may happen).
...
\begin{theorem} \label{thm:DL-Sn-am}
  ...
  $$
  %n + \binom{n}{2}(a - 1) - S(n, a) \,.
  n + \frac{(n - 1)n}{2} (a - 1)
  $$
  modulo ${(a - 1)}^2$.
  % there exists $q \in \bZ$ such that 
  % $$
 ...
 \begin{proof}
   Let $T\colon \bZ \times \bZ \times \bZ \to \bZ$ be defined by:
   $$
   T(n, a, q) = n + \frac{(n - 1)n}{2} (a - 1) + q {(a - 1)}^2
   $$
   % and
   % $$
   % Q(n, a, q) = \frac{(n - 1)n}{2} + q a  
   % $$
   for every $n$, $a$, $q \in \bZ$.
   Our task is to prove that for each $(n, a) \in \bN \times \bZ$,
   there exists $q \in \bZ$ such that $S(n, a) =  T(n, a, q)$.
   We proceed by induction on~$n$.
   Since $S(0, a) = 0 = T(0, a, 0)$ for every $a \in \bZ$, the basis of our induction holds true.
   Besides, straightforward computations show that for every $a$, $n$, $q \in \bZ$,
   the tri-variate polynomial function $T$ satisfy
   % \begin{equation} \label{eq:aTnaq-TnqQ}
   $$
   a T(n, a, q) + 1 = T(n + 1, a, q') \,, 
   $$
   with 
   \begin{equation} \label{eq:def-qprime}
   q' = \frac{(n - 1)n}{2} + a q \,. 
   \end{equation} 
   % = 1 + T(n, a, q) + (a - 1) T(n, a, q) = 
%   \end{equation} 
   Therefore, for every $n \in \bN$ and every $a$, $q \in \bZ$ such that $S(n, a) = T(n, a, q)$,
   $S(n + 1, a)$ can be written as $S(n + 1, a) = T(n + 1, a, q')$, 
   where $q'$ is given by Equation~\eqref{eq:def-qprime}.
   Hence, the inductive steps holds true.
   \end{proof}
 \begin{exercise}
 \begin{enumerate}
 \item Let $f$ be a polynomial with integer coefficients and let $k \in \bZ$. 
   Prove that $f(k)$ is congruent of $f(0) + f'(0) k$ modulo $k^2$. 
 \item Let $a \in \bZ$ and let $m$, $n \in \bN$.
   Prove that $S(n, a^m)$ is congruent to
  $$ 
  n + m \frac{(n - 1)n}{2} (a - 1)
  $$
  modulo ${(a - 1)}^2$.
 \end{enumerate}
\end{exercise}
