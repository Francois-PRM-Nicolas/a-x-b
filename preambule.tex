...
$$
a^n   = {(1 + p q)}^n = \sum_{k = 0}^n \binom{n}{k} p^k q^k
$$
$$
a^n - 1 = n p q + \sum_{k = 2}^n \binom{n}{k} p^k q^k
$$

$$
S(n, a) = \frac{a^n - 1}{a - 1} = \frac{1}{pq} (a^n - 1) = n + \sum_{k = 2}^n \binom{n}{k} p^{k - 1} q^{k - 1}
$$

$$
p^{k - 1} \binom{n}{k} = \frac{n p^{k - 1}}{k} \binom{n - 1}{k - 1}
$$

\begin{lemma}
  For every $k$, $p \in \bZ_{\ge 2}$,
  $p^{k - 1} \le k$ is equivalent to $k = p = 2$.
\end{lemma}
...
$$
% $$
% F(t + h) = p^h \frac{1}{1 + ht^{-1}} F(t) 
% $$
$$
\frac{2}{k} 
= \prod_{j = 3}^{k}  \frac{j - 1}{j}
= \prod_{j = 3}^{k}  \left( 1 - j^{-1} \right)
\ge  \left( \frac{2}{3} \right)^{k - 2} 
$$

$$
\frac{p^{k - 1}}{k} = \frac{2^{k - 1}}{k} \ge \left( \frac{4}{3} \right)^{k - 2} > 1
$$


$$
F(k, p)
= \frac{p^k}{2}  \prod_{j = 3}^{k}  \frac{j - 1}{j}
= \prod_{j = 2}^{k} p  \left( 1 - j^{-1} \right)
= \frac{p}{2} \prod_{j = 3}^{k} p  \left( 1 - j^{-1} \right)
$$
...
If $k \ge 2$ 
$$
\frac{F(k + 1)}{F(k)} \ge \frac{2p}{3}  \ge \frac{4}{3} 
$$
\begin{proof}
